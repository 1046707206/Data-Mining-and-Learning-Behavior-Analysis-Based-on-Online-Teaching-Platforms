...
def get_metrics(real_score, predict_score):     #计算指标的函数
    ...
    thresholds = sorted_predict_score[(np.array([sorted_predict_score_num])*np.arange(1, 1485)/np.array([1485])).astype(int)]
...
os.system('bash main.py')   #执行bash命令进行训练，windows下可直接在main.py中设置参数直接"python main.py"
...
for i in range(100):
    print("用户"+str(zeroindex[0][index[i]])+"可能会选择的课程"+"course"+str(zeroindex[1][index[i]]))
